...
async def generate_nutrition(
    ...
    try:
        ...
        # Enrich plan with real Spoonacular recipes for some days
        try:
            recipes = await spoonacular_service.get_recommended_recipes(
                diet=user_data["diet_preference"],
                calories=data.calorie_target // 3
            )
            if recipes and "days" in plan:
                for i, recipe in enumerate(recipes):
                    if i < len(plan["days"]):
                        plan["days"][i]["lunch"]["name"] = recipe["title"]
                        plan["days"][i]["lunch"]["image"] = recipe["image"]
                        plan["days"][i]["lunch"]["source_url"] = recipe["sourceUrl"]
        except Exception as e:
            print(f"Spoonacular enrichment error: {e}")
