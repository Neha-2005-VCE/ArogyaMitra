...
def get_history(
    ...
    return {"history": [
        {"date": str(r.date), "calories_burned": r.calories_burned,
         ...
         "weight": r.weight, "body_fat_percentage": r.body_fat_percentage, "notes": r.notes}
        for r in records
    ]}
...
def log_progress(
    ...
    if record:
        ...
        if data.body_fat_percentage is not None: record.body_fat_percentage = data.body_fat_percentage
    ...
    if data.calories_burned:
        current_user.charity_donations = (current_user.charity_donations or 0) + int((data.calories_burned or 0) // 10)
...
def get_achievements(
    ...
    for a in achievements:
        ...
        a["percentage"] = min(100, round((current / target) * 100)) if target > 0 else 0
...
def get_charts(
    ...
    calories_chart = [
        {"date": str(d), "calories": records_by_date[d].calories_burned if d in records_by_date else 0}
        for d in last_7
    ]
    streak_chart = [
